...
#define WATCHDOG_NAME		"Matrix"
#define PFX			WATCHDOG_NAME ": "
#define DRIVER_VERSION		WATCHDOG_NAME " driver, v" WATCHDOG_VERSION "\n"
#define WD_MAGIC		'V'

#define MATRIX_MINOR        241
...
#define ACTREG		0x30
#define BASEREG 	0x60
#define SIMPLE_IO_BASE_ADDR    0x62 /* index in LDN_GPIO_WDT */
...
/* GP LED Blanking 1 Registers LDN=0x07 */
#define LEDCFG_INDEX 	   0xF8   /* Pin Mapping Register */
#define LEDCTRL_INDEX 	   0xF9
...
static void wdt_keepalive(void)
{
	...
	{
		inb(base);
		printk(KERN_CRIT PFX "wdt_keepalive 1!\n");
	}
	else
	{
		/* The timer reloads with around 5 msec delay */
		outb(0x55, CIR_DR(base));
		/*printk(KERN_CRIT PFX "wdt_keepalive 2!\n");*/
	}
...
static void wdt_start(void)
{
	...
	superio_outb(timeout, WDTVALLSB);
        ...
        //superio_outb( 0x2E, LEDCFG_INDEX ); /* GP56/pin-60 */
        //superio_outb( 0x2, LEDCTRL_INDEX ); /* enable 1Hz LED blink  */   
...
static void wdt_stop(void)
{
	...
	//superio_outb( 0x0, LEDCFG_INDEX );  /* default setting */
       // superio_outb( 0x0, LEDCTRL_INDEX ); /* default setting */    
...
static long wdt_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case DAS_IOC_GET_GPI :	
		if( base_addr != 0 )
		{	
			temp_param = (void *)kmalloc( sizeof( unsigned long ), GFP_ATOMIC );
			errcode=copy_from_user ((void *)temp_param, (const void*)arg, sizeof( unsigned long ));

			bTemp = inb( (base_addr + 1) ); /* High-part of Simple IO Set 2, DI0 ~ DI3 */
			wValue = ( (bTemp>> 4) & 0x0F );
			*((unsigned long *)temp_param) = (unsigned long)wValue;
			errcode=copy_to_user((void *)arg, (const void *)temp_param, sizeof( unsigned long ));
			kfree( temp_param );
		}
	...
	case DAS_IOC_SET_GPO :	
		...
		if( base_addr!= 0 )
		{
			...
			wValue = (*((unsigned long *)temp_param) & 0x0F);

			outb( (u8)wValue , (base_addr + 4));    /* Simple IO Set 5	*/


			bTemp = inb( (base_addr + 4) ); /* Simple IO Set 5	*/
			wValue = ( (bTemp) & 0x0F );
			*((unsigned long *)temp_param) = (unsigned long )wValue;
			errcode=copy_to_user((void *)arg, (const void *)temp_param, sizeof( unsigned long  ));
			kfree( temp_param );
...
static struct miscdevice wdt_miscdev = {
	...
	.minor		= MATRIX_MINOR, //WATCHDOG_MINOR,
	.name		= "matrix",       //"watchdog",
	.fops		= &wdt_fops,
};
...
static int __init it87_wdt_init(void)
{
    ...
    printk(KERN_ERR PFX
           "Chip found, Chip %04x Revision %02x\n",
           chip_type, chip_rev);
...
/* Add GPIO Code */
        ...
        t = superio_inb( 0x26 ); /* Configure GP24~27 in GPIO Set2 */
        t = (t | 0xF0); /* GP24~27 */
        superio_outb( t, 0x26 ); 

        t = superio_inb( 0x29 ); /* Configure GP50~53 in GPIO Set5 */
        t = (t | 0x0F); /* GP50~53 */
        superio_outb( t, 0x29 );


        t = superio_inb( 0xC1 ); /* Configure GP24~27 in GPIO Set2 */
        t = (t | 0xF0); /* GP24~27 */
        superio_outb( t, 0xC1 ); /* Simple I/O Set 2 Enable register (Bit 7-0 powered by VCCH) */


        t = superio_inb( 0xC4 ); /* Configure GP50~53 in GPIO Set5 */
        t = (t | 0x0F); /* GP50~53 */
        superio_outb( t, 0xC4 ); /* Simple I/O Set 5 Enable register (Bit 7-0 powered by VCCH) */
                
        t = superio_inb( 0xC9 ); /* Configure GP24~27 in GPIO Set2 */
        t = (t & 0x0F); /* t = (t & (~0x0F) ), GP24~27 */
        superio_outb( t, 0xC9 ); /* Simple I/O Set 2 Output Enable register */
                        
        t = superio_inb( 0xCC ); /* Configure GP50~53 in GPIO Set5 */
        t = (t | 0x0F); /* GP50~53 */
        superio_outb( t, 0xCC ); /* Simple I/O Set 5 Output Enable register (Bit 7-0 powered by VCCH) */
         
  
        t = superio_inb( 0xB1 ); /*  Configure GP24~27 in GPIO Set2 Invert Parity */
        t = (t | 0xF0); 
        superio_outb( t, 0xB1 ); 

        t = superio_inb( 0xB4 ); /* Configure GP50~53 in GPIO Set5 Invert Parity */
        t = (t | 0x0F); 
        superio_outb( t, 0xB4 ); 

	base_addr = superio_inw( SIMPLE_IO_BASE_ADDR );

/* /////////////////////////////////////////////////////////////////////// Clear GO  */
		wValue = 0;
				
		outb((u8)wValue, (base_addr + 4));  /* Simple IO Set 5	Clear GO */
		
		
	/*	bTemp = inb(base_addr + 1);  */ /*  Simple IO Set 2 */

	/*	wValue = (bTemp & (~0x08));  */ /* GPIO enable (GP23) low acitve */

	/*	outb((u8)wValue, (base_addr + 1));    */ /*  Simple IO Set 2 */
/* ////////////////////////////////////////////////////////////////////// */



/* /////////////////////////////////////////////////// on MXE-1300, GP56 is WDT LED  */
       // t = superio_inb( 0x29 ); /* Configure Pin-60 as GP56 (LED) in GPIO Set5 */
        //t = (t | 0x40); 
       // superio_outb( t, 0x29 ); 
    
/* 2010Dec31, recommended by Kurt */
       // t = superio_inb( 0xC4 ); /* Simple I/O Set 5 Enable register, disable GP56 Simple I/O */
       // t = (t & (~0x40)); 
       // superio_outb( t, 0xC4 );        
/* end, 2010Dec31, recommended by Kurt */ 


        /* // This setting is configured in BIOS */
       // t = superio_inb( 0xBC ); /* Enable Pull-up on GP56 */
        //t = (t | 0x40); 
       // superio_outb( t, 0xBC );               
...
MODULE_LICENSE("GPL");
MODULE_ALIAS_MISCDEV(MATRIX_MINOR); // WATCHDOG_MINOR);
